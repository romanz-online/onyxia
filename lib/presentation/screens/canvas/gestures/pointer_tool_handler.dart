...
class PointerToolHandler extends CanvasToolGestureHandler {
  PointerToolHandler({required super.canvasConfig});
      ...
      (details, ref, buildContext, interactionContext) {
        switch (interactionContext) {
          ...
          case ArrowWellInteraction(:final sourceObject, :final connectionPoint):
            ...
            ArrowInteractionService.autoCompleteArrow(sourceObject, connectionPoint, ref, buildContext);
            break;
      ...
      (details, ref, buildContext, interactionContext) {
        switch (interactionContext) {
          case ArrowWellInteraction(:final sourceObject, :final connectionPoint):
            ...
            ref.read(canvasGestureStateProvider.notifier).setArrowMoveType(ArrowMoveType.end);
            break;
          case ObjectResizeInteraction(:final targetObject, :final handle):
            ref.read(canvasGestureStateProvider.notifier).setActiveObject(targetObject);
          ...
          case ArrowMoveInteraction(:final targetObject, :final moveType):
            ref.read(canvasGestureStateProvider.notifier).setActiveObject(targetObject);
            ref.read(canvasGestureStateProvider.notifier).setArrowMoveType(moveType);
          ...
          case BackgroundInteraction():
            ref.read(canvasObjectsProvider.notifier).clearSelectedObjects();
            ref.read(dragSelectProvider.notifier).startDragSelect(details.localPosition);
            break;
          case _:
            break;
        }
      ...
      (details, ref, buildContext, interactionContext) {
        switch (interactionContext) {
          ...
          case ArrowResizeInteraction(:final targetObject, :final segmentIndex):
            ...
            break;
          case ArrowMoveInteraction(:final targetObject, :final moveType):
            ArrowInteractionService.updateArrowPan(details, ref, targetObject, part: moveType);
            break;
          case ObjectFillInteractionContext():
            _updateObjectMovePan(details, ref);
            break;
          case BackgroundInteraction():
            ref.read(dragSelectProvider.notifier).updateDragSelect(details.localPosition);
            break;
          case _:
            break;
        }
  ...
  void Function(DragEndDetails, WidgetRef, BuildContext, CanvasInteractionContext)? get onPanEnd =>
      (details, ref, buildContext, interactionContext) {
        ...
        if (interactionContext is ArrowWellInteraction || interactionContext is ArrowMoveInteraction) {
  ...
  @override
  void Function(TapDownDetails, WidgetRef, BuildContext, CanvasInteractionContext)? get onSecondaryTapDown =>
      (details, ref, buildContext, interactionContext) {
        switch (interactionContext) {
          case ObjectFillInteractionContext(:final targetObject):
            canvasRightClick(
  ...
  @override
  void Function(PointerHoverEvent, WidgetRef, BuildContext, CanvasInteractionContext)? get onHover =>
      (event, ref, buildContext, interactionContext) {
          ...
          case ObjectResizeInteraction(:final targetObject, :final handle):
            if (selectedObjects.contains(targetObject)) {
              final cursor = switch (handle) {
                ...
                ResizeHandle.topCenter || ResizeHandle.bottomCenter => SystemMouseCursors.resizeUpDown,
                ResizeHandle.centerLeft || ResizeHandle.centerRight => SystemMouseCursors.resizeLeftRight,
            ...
            break;
          case ObjectFillInteractionContext(:final targetObject):
            if (selectedObjects.contains(targetObject) && !targetObject.isBrush && !targetObject.isArrow) {
  ...
  void _canvasObjectTapUp(CanvasObject? object, WidgetRef ref) {
    ...
    if (CanvasInteractionService.isModifierPressed()) {
      ...
      final wasSelected = ref.read(canvasObjectsProvider).selectedObjects.contains(object);
      ref.read(canvasObjectsProvider.notifier).clearSelectedObjects();
      ref.read(canvasObjectsProvider.notifier).selectObject(object);
  ...
  void _startObjectMovePan(Offset position, CanvasObject object, WidgetRef ref) {
  ...
  void _updateObjectMovePan(DragUpdateDetails details, WidgetRef ref) {
    ...
    final gridDelta = ref.read(canvasSettingsProvider(Setting.snapToGrid))
        ? ref.read(canvasBoundsProvider.notifier).snap(ref.read(canvasGestureStateProvider).accumulatedDelta)
        : ref.read(canvasGestureStateProvider).accumulatedDelta;
    final deltaToUse = ref.read(canvasSettingsProvider(Setting.snapToGrid)) ? gridDelta : details.delta;

    if (deltaToUse != Offset.zero) {
      // Get movement constraints for all selected objects
      List<Map<String, bool>> allConstraints =
          selectedObjects.map((obj) => obj.getMovementConstraints(ref, deltaToUse)).toList();

      // Calculate which directions ALL objects can move
      bool allCanMoveUp =
          deltaToUse.dy < 0 ? allConstraints.every((constraints) => constraints['canMoveUp'] == true) : true;
      bool allCanMoveDown =
          deltaToUse.dy > 0 ? allConstraints.every((constraints) => constraints['canMoveDown'] == true) : true;
      bool allCanMoveLeft =
          deltaToUse.dx < 0 ? allConstraints.every((constraints) => constraints['canMoveLeft'] == true) : true;
      bool allCanMoveRight =
          deltaToUse.dx > 0 ? allConstraints.every((constraints) => constraints['canMoveRight'] == true) : true;
      ...
      final constrainedDelta = Offset(
        ...
        (deltaToUse.dx > 0 && allCanMoveRight) || (deltaToUse.dx < 0 && allCanMoveLeft) ? deltaToUse.dx : 0,
        // Vertical movement: only apply dy if movement direction is allowed
        (deltaToUse.dy > 0 && allCanMoveDown) || (deltaToUse.dy < 0 && allCanMoveUp) ? deltaToUse.dy : 0,
      );
      ...
      if (constrainedDelta != Offset.zero) {
        ...
        for (final selectedObj in selectedObjects.where((e) => !e.isArrow)) {
          for (final arrow in ref.read(canvasObjectsProvider).objects.where((e) => e.isArrow)) {
...

            {
              ...
              if (obj.id.isNotEmpty && !selectedObjects.contains(obj)) {
        ...
        ref.read(canvasObjectsProvider.notifier).updateObjectsState(objectsToUpdate);
        ref.read(canvasGestureStateProvider.notifier).updateAccumulatedDelta(-gridDelta);
      }
    }
  }
  ...
  void _updateObjectResizePan(
    ...
    if (!ref.read(canvasObjectsProvider).selectedObjects.contains(object)) return;
  ...
  void _startArrowResize(
    ...
    if (!ref.read(canvasObjectsProvider).selectedObjects.contains(arrow)) return;

    ref.read(canvasGestureStateProvider.notifier).setArrowSegmentIndex(segmentIndex);
  }
  ...
  void _handleArrowResize(
    ...
    if (deltaToUse != Offset.zero) {
      final newSegmentCreated = arrow.handleResize(
        ...
        deltaToUse,
        ResizeHandle.arrow,
        arrowKeypointIndex: ref.read(canvasGestureStateProvider).arrowSegmentIndex ?? 0,
      );
  ...
  void _handleArrowTextDrag(
    ...
    arrow.updateTextPosition(ref.read(canvasViewportProvider.notifier).convertToCanvasCoords(details.globalPosition));
    ref.read(canvasObjectsProvider.notifier).updateObjectState(arrow);
  }
  ...
  void _pruneArrowKeypoints(WidgetRef ref) {
    for (final obj in ref.read(canvasObjectsProvider).objects.where((e) => e.isArrow)) {
      obj.pruneKeypoints();
    }
  }
}
