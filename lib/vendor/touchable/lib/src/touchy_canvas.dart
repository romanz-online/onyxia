...
class TouchyCanvas {
  ...
  // Getter to access ShapeHandler from external classes
  ShapeHandler get shapeHandler => _shapeHandler;
  ...
  dynamic hitTestForDropEvent(Offset point) {
    final touchedShapes = _shapeHandler.getTouchedShapes(point);
    if (touchedShapes.isEmpty) return null;

    // Return the context of the topmost shape (first in the list)
    return _shapeHandler.getContextForShape(touchedShapes.first);
  }
