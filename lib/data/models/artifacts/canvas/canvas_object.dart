...
const Size defaultArtifactObjectDimensions = Size(CanvasBounds.gridSpacing * 10, CanvasBounds.gridSpacing * 10);
...
class CanvasObject {
  ...
  factory CanvasObject.fromJson(String jsonStr) => CanvasObject.fromMap(json.decode(jsonStr));
    ...
    switch (type) {
      ...
      case CanvasObjectType.arrow:
        ...
        if (arrowProps.arrowType == ArrowType.curved) {
          ...
          if (point.dx < minX || point.dx > maxX || point.dy < minY || point.dy > maxY) {
            return false;
          }
          ...
          for (int i = 0; i < arrowProps.points.length - 1; i++) {
            ...
            if (l2 > 0) {
              ...
              final t = math.max(0, math.min(1, (point - p1).dot(p2 - p1) / l2));
  ...
  (ConnectionPoint, Offset) _findNearestBoundingBoxOffset(Offset position) {
    ...
    final minDistance =
        [distanceToTop, distanceToBottom, distanceToLeft, distanceToRight].reduce((a, b) => a < b ? a : b);
  ...
  (ConnectionPoint, Offset) _findNearestDiamondOffset(Offset position) {
    ...
    final distances = [distToTopRight, distToRightBottom, distToBottomLeft, distToLeftTop];
    final minDistance = distances.reduce((a, b) => a < b ? a : b);

    if (minDistance == distToTopRight) {
      return (ConnectionPoint.top, _projectOntoLineSegment(position, top, right));
    } else if (minDistance == distToRightBottom) {
      return (ConnectionPoint.right, _projectOntoLineSegment(position, right, bottom));
    } else if (minDistance == distToBottomLeft) {
      return (ConnectionPoint.bottom, _projectOntoLineSegment(position, bottom, left));
    } else {
      return (ConnectionPoint.left, _projectOntoLineSegment(position, left, top));
    }
  }
  ...
  (ConnectionPoint, Offset) _findNearestOblongOffset(Offset position) {
    ...
    if (rect.width <= rect.height) {
      ...
      if (position.dy >= topSemicircleBottom && position.dy <= bottomSemicircleTop) {
        ...
        final projectedPoint =
            _projectOntoSemicircle(position, Offset(center.dx, rect.top + effectiveRadius), effectiveRadius, true);
        ...
        final projectedPoint =
            _projectOntoSemicircle(position, Offset(center.dx, rect.bottom - effectiveRadius), effectiveRadius, false);
      ...
      // Determine which region the position is in
      if (position.dx >= leftSemicircleRight && position.dx <= rightSemicircleLeft) {
        // Position is in the straight edge region (between the two semicircles)
        ...
        final projectedPoint = _projectOntoSemicircle(
            position, Offset(rect.left + effectiveRadius, center.dy), effectiveRadius, true,
            isVertical: true);
        ...
        final projectedPoint = _projectOntoSemicircle(
            position, Offset(rect.right - effectiveRadius, center.dy), effectiveRadius, false,
            isVertical: true);
  ...
  (ConnectionPoint, Offset) _findNearestRhombusOffset(Offset position) {
    ...
    if (minDistance == distToTop) {
      return (ConnectionPoint.top, _projectOntoLineSegment(position, topL, topR));
    } else if (minDistance == distToRight) {
      return (ConnectionPoint.right, _projectOntoLineSegment(position, topR, bottomR));
    } else if (minDistance == distToBottom) {
      return (ConnectionPoint.bottom, _projectOntoLineSegment(position, bottomR, bottomL));
    } else {
      return (ConnectionPoint.left, _projectOntoLineSegment(position, bottomL, topL));
    }
  }
  ...
  (ConnectionPoint, Offset) _findNearestHouseOffset(Offset position) {
    ...
    // Calculate distances to each edge
    final distToLeftRoof = _distanceToLineSegment(position, roofBottomLeft, roofPeak);
    final distToRightRoof = _distanceToLineSegment(position, roofPeak, roofBottomRight);
    final distToLeftWall = _distanceToLineSegment(position, roofBottomLeft, houseBottomLeft);
    final distToRightWall = _distanceToLineSegment(position, roofBottomRight, houseBottomRight);
    final distToBottom = _distanceToLineSegment(position, houseBottomLeft, houseBottomRight);

    final distances = [distToLeftRoof, distToRightRoof, distToLeftWall, distToRightWall, distToBottom];
    final minDistance = distances.reduce((a, b) => a < b ? a : b);
    ...
    if (minDistance == distToLeftRoof) {
      final projectedPoint = _projectOntoLineSegment(position, roofBottomLeft, roofPeak);
      ...
      final projectedPoint = _projectOntoLineSegment(position, roofPeak, roofBottomRight);
      // If close to peak, snap to peak; otherwise use projected point on diagonal
      if (distanceToPeak <= peakRadius) {
        return (ConnectionPoint.top, roofPeak);
      } else {
      ...
      return (ConnectionPoint.left, _projectOntoLineSegment(position, roofBottomLeft, houseBottomLeft));
    } else if (minDistance == distToRightWall) {
      return (ConnectionPoint.right, _projectOntoLineSegment(position, roofBottomRight, houseBottomRight));
    } else {
      return (ConnectionPoint.bottom, _projectOntoLineSegment(position, houseBottomLeft, houseBottomRight));
    }
  }
  ...
  (ConnectionPoint, Offset) _findNearestReverseHouseOffset(Offset position) {
    ...
    // Calculate distances to each edge
    final distToTop = _distanceToLineSegment(position, houseTopLeft, houseTopRight);
    final distToLeftWall = _distanceToLineSegment(position, houseTopLeft, roofTopLeft);
    final distToRightWall = _distanceToLineSegment(position, houseTopRight, roofTopRight);
    final distToLeftRoof = _distanceToLineSegment(position, roofTopLeft, roofPeak);
    final distToRightRoof = _distanceToLineSegment(position, roofPeak, roofTopRight);

    final distances = [distToTop, distToLeftWall, distToRightWall, distToLeftRoof, distToRightRoof];
    final minDistance = distances.reduce((a, b) => a < b ? a : b);
    ...
    if (minDistance == distToTop) {
      return (ConnectionPoint.top, _projectOntoLineSegment(position, houseTopLeft, houseTopRight));
    } else if (minDistance == distToLeftWall) {
      return (ConnectionPoint.left, _projectOntoLineSegment(position, houseTopLeft, roofTopLeft));
    } else if (minDistance == distToRightWall) {
      return (ConnectionPoint.right, _projectOntoLineSegment(position, houseTopRight, roofTopRight));
    } else if (minDistance == distToLeftRoof) {
      final projectedPoint = _projectOntoLineSegment(position, roofTopLeft, roofPeak);
      // If close to peak, snap to peak; otherwise use projected point on diagonal
      if (distanceToPeak <= peakRadius) {
      ...
      final projectedPoint = _projectOntoLineSegment(position, roofPeak, roofTopRight);
      // If close to peak, snap to peak; otherwise use projected point on diagonal
      if (distanceToPeak <= peakRadius) {
        return (ConnectionPoint.bottom, roofPeak);
      } else {
  ...
  double _distanceToLineSegment(Offset point, Offset lineStart, Offset lineEnd) {
    ...
    final t = math.max(0, math.min(1, (point - lineStart).dot(lineEnd - lineStart) / lineLength));
    final projection = lineStart + (lineEnd - lineStart) * t.toDouble();
    return (point - projection).distance;
  }

  /// Helper method to project point onto line segment
  Offset _projectOntoLineSegment(Offset point, Offset lineStart, Offset lineEnd) {
    final lineLength = (lineEnd - lineStart).distanceSquared;
  ...
  Offset _projectOntoSemicircle(Offset point, Offset semicircleCenter, double radius, bool isTop,
    ...
    if (isVertical) {
      ...
      if (isTop) {
        ...
        final length = math.sqrt(constrainedX * constrainedX + normalizedY * normalizedY);
        if (length == 0) return semicircleCenter + Offset(-radius, 0);
        return semicircleCenter + Offset((constrainedX / length) * radius, (normalizedY / length) * radius);
      } else {
      ...
      if (isTop) {
        ...
        final length = math.sqrt(normalizedX * normalizedX + constrainedY * constrainedY);
        if (length == 0) return semicircleCenter + Offset(0, -radius);
        return semicircleCenter + Offset((normalizedX / length) * radius, (constrainedY / length) * radius);
      } else {
  ...
  bool handleResize(
    ...
    if (isArrow) {
      ...
      bool isHorizontal = newKeypoints[arrowKeypointIndex].dy - newKeypoints[arrowKeypointIndex + 1].dy == 0;

      // only allows orthogonal movement depending on segment orientation
      Offset appliedDelta = isHorizontal ? Offset(0, delta.dy) : Offset(delta.dx, 0);

      if (newKeypoints.length == 2) {
        // straight line (one segment)
        final midpoint1 = _getOffsetBetween(newKeypoints[0], newKeypoints[1], minSegmentLength);
        final midpoint2 = _getOffsetBetween(newKeypoints[1], newKeypoints[0], minSegmentLength);
        ...
        newKeypoints[2] = canvasBoundsNotifier.clamp(newKeypoints[2] + appliedDelta);
        newKeypoints[3] = canvasBoundsNotifier.clamp(newKeypoints[3] + appliedDelta);
      } else if (isFirstSegment || isLastSegment) {
        double segmentLength = (newKeypoints[arrowKeypointIndex + 1] - newKeypoints[arrowKeypointIndex]).distance;

        // Only allow creating new segments if the current one is longer than minimum
        if (segmentLength > minSegmentLength) {
          if (isFirstSegment) {
            final newKeypoint = _getOffsetBetween(newKeypoints[0], newKeypoints[1], minSegmentLength);
            ...
            final newKeypoint = _getOffsetBetween(
                newKeypoints[newKeypoints.length - 1], newKeypoints[newKeypoints.length - 2], minSegmentLength);

            newKeypoints.insert(newKeypoints.length - 1, newKeypoint);
            newKeypoints.insert(newKeypoints.length - 1, newKeypoint);

            // Apply delta and clamp to canvas bounds
            newKeypoints[newKeypoints.length - 3] =
                canvasBoundsNotifier.clamp(newKeypoints[newKeypoints.length - 3] + appliedDelta);
            newKeypoints[newKeypoints.length - 4] =
                canvasBoundsNotifier.clamp(newKeypoints[newKeypoints.length - 4] + appliedDelta);
          }
        }
      } else {
        // Apply delta and clamp to canvas bounds
        newKeypoints[arrowKeypointIndex] = canvasBoundsNotifier.clamp(newKeypoints[arrowKeypointIndex] + appliedDelta);
        newKeypoints[arrowKeypointIndex + 1] =
            canvasBoundsNotifier.clamp(newKeypoints[arrowKeypointIndex + 1] + appliedDelta);
      }
      ...
      if (topLeft != newTopLeft && snapToGrid && type != CanvasObjectType.image) {
        newTopLeft = canvasBoundsNotifier.snap(newTopLeft);
      }

      if (bottomRight != newBottomRight && snapToGrid && type != CanvasObjectType.image) {
      ...
      if ((newTopLeft.dx - newBottomRight.dx).abs() < CanvasBounds.gridSpacing) {
        newTopLeft = Offset(topLeft.dx, newTopLeft.dy);
        newBottomRight = Offset(bottomRight.dx, newBottomRight.dy);
      }

      if ((newTopLeft.dy - newBottomRight.dy).abs() < CanvasBounds.gridSpacing) {
  ...
  bool canMove(WidgetRef ref, Offset delta) {
    ...
    if (isArrow) {
      ...
      Offset newTopLeft = snapToGrid ? canvasBoundsNotifier.snap(topLeft + delta) : topLeft + delta;
  ...
  bool handleMove(WidgetRef ref, Offset delta) {
    ...
    if (isArrow) {
      arrowProps.points =
          arrowProps.points.map((p) => snapToGrid ? canvasBoundsNotifier.snap(p + delta) : p + delta).toList();
      return true;
    } else if (isBrush) {
      ...
      topLeft = snapToGrid ? canvasBoundsNotifier.snap(topLeft + delta) : topLeft + delta;
    ...
    switch (type) {
      ...
      case CanvasObjectType.oblong:
        ...
        if ((rect.width - rect.height).abs() <= CanvasBounds.gridSpacing) {
          ...
          final innerSize = radius * math.sqrt(2); // Distance from center to quarter-circle center
          return Rect.fromCenter(
            center: center,
            width: innerSize,
            height: innerSize,
        ...
        if (rect.width <= rect.height) {
          ...
          final innerBottom = rect.bottom - effectiveRadius + quarterCircleOffset;
...
extension GenericCanvasObjectExtension on CanvasObject {
  bool get isGeneric =>
      type != CanvasObjectType.arrow && type != CanvasObjectType.image && type != CanvasObjectType.brush;
}
...
extension ArrowCanvasObjectExtension on CanvasObject {
  ...
  CanvasObject getStartObject(WidgetRef ref) => ref.read(canvasObjectsProvider).objects.firstWhere(
        (e) => e.id == arrowProps.startObjectId,
        orElse: () => CanvasObject.initial(),
      );

  CanvasObject getEndObject(WidgetRef ref) => ref.read(canvasObjectsProvider).objects.firstWhere(
  ...
  Offset getStartOffset(WidgetRef ref, {CanvasObject? startObject}) {
    ...
    return (startObj.id.isEmpty || arrowProps.startPoint == ConnectionPoint.none)
  ...
  void drawNewKeypoints(
    ...
    arrowProps.curvedMidpoint = arrowProps.points.isEmpty ? curveOffset(0.0) : curveOffset(0.5);
  ...
  void handleMoveEndPoint(
    ...
    final snapToGrid = ref.read(canvasSettingsProvider(Setting.snapToGrid));
    final canvasBoundsNotifier = ref.read(canvasBoundsProvider.notifier);
    if (endObj.id.isEmpty || endObj.id == arrowProps.startObjectId || startObj.isPointInObject(position)) {
      arrowProps.endObjectId = null;
      arrowProps.endPoint = ConnectionPoint.none;
      arrowProps.endAbsoluteOffset = snapToGrid ? canvasBoundsNotifier.snap(position) : position;
    } else {
      ...
      arrowProps.endRelativeOffset = arrowProps.endAbsoluteOffset! - connectionPointOffset;

      if (arrowProps.endRelativeOffset!.distance <= CanvasBounds.gridSpacing * 2) {
  ...
  void handleMoveStartPoint(
    ...
    final snapToGrid = ref.read(canvasSettingsProvider(Setting.snapToGrid));
    final canvasBoundsNotifier = ref.read(canvasBoundsProvider.notifier);
    if (startObj.id.isEmpty || startObj.id == arrowProps.endObjectId || endObj.isPointInObject(position)) {
      arrowProps.startObjectId = null;
      arrowProps.startPoint = ConnectionPoint.none;
      arrowProps.startAbsoluteOffset = snapToGrid ? canvasBoundsNotifier.snap(position) : position;
    } else {
      ...
      arrowProps.startRelativeOffset = arrowProps.startAbsoluteOffset! - connectionPointOffset;

      if (arrowProps.startRelativeOffset!.distance <= CanvasBounds.gridSpacing * 2) {
  ...
  void updateTextPosition(Offset cursorPosition) {
    ...
    for (int i = 0; i < arrowProps.points.length - 1; i++) {
      ...
      if (segmentLength > 0) {
        ...
        final t = (cursorVector.dx * segmentVector.dx + cursorVector.dy * segmentVector.dy) /
            (segmentVector.dx * segmentVector.dx + segmentVector.dy * segmentVector.dy);
        ...
        if (distance < closestDistance) {
          ...
          bestPosition = (accumulatedLength + segmentLength * tClamped) / totalLength;
        }
      }
  ...
  void adjustPointsToObject(CanvasObject anchor) {
    ...
    if (arrowProps.startObjectId == anchor.id) {
      ...
      updatedKeypoints[0] = arrowProps.startPoint.getOffset(anchor) + (arrowProps.startRelativeOffset ?? Offset.zero);
      bool isHorizontal =
          arrowProps.startPoint == ConnectionPoint.left || arrowProps.startPoint == ConnectionPoint.right;
      ...
      if (originalKeypoints.length >= 3) {
        ...
        final conflict = (isHorizontal && dy > dx) || (!isHorizontal && dx > dy);
        ...
        if (conflict) {
          ...
          final turnPoint =
              isHorizontal ? Offset(origP1.dx, updatedKeypoints[0].dy) : Offset(updatedKeypoints[0].dx, origP1.dy);
          updatedKeypoints.insert(1, turnPoint);
        } else {
          updatedKeypoints[1] = Offset(isHorizontal ? updatedKeypoints[1].dx : updatedKeypoints[0].dx,
      ...
      updatedKeypoints[updatedKeypoints.length - 1] =
          arrowProps.endPoint.getOffset(anchor) + (arrowProps.endRelativeOffset ?? Offset.zero);
      bool isHorizontal = arrowProps.endPoint == ConnectionPoint.left || arrowProps.endPoint == ConnectionPoint.right;
  ...
  Offset getTextOffset() {
    ...
    for (int i = 0; i < arrowProps.points.length - 1; i++) {
      final segmentLength = (arrowProps.points[i + 1] - arrowProps.points[i]).distance;
    ...
    final targetDistance = totalLength * arrowProps.textPosition.clamp(0.0, 1.0);
    ...
    for (int i = 0; i < segmentLengths.length; i++) {
      ...
      if (accumulatedDistance + segmentLength >= targetDistance) {
        ...
        return Offset.lerp(arrowProps.points[i], arrowProps.points[i + 1], t) ?? arrowProps.points[i];
      }
