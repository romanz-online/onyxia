...
class ArtifactsTreeNotifier extends StreamNotifier<List<Artifact>> {
  ...
  /// (including the renamed item's own content) — body.content is canonical, so
  /// persisting it is all that's needed; an open editor on this device picks up
  /// the rewrite via the noteEditorState content backstop, and other clients via
  /// the artifacts realtime stream. Navigates the URL if the user is currently
  /// viewing this item. Returns `null` on success or no-op (empty / unchanged name).
